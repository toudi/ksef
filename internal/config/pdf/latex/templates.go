package latex

import (
	"errors"
	"fmt"
	"path/filepath"
	"strings"
	"text/template"
	"time"

	"github.com/spf13/viper"
)

var (
	errNoTemplatesDefined = errors.New("nie zdefiniowano żadnych szablonów renderujących")
)

type HeaderFooterSettings struct {
	Left   string
	Center string
	Right  string
}

type templatesCollection struct {
	Enabled  bool
	SrcPath  string
	Header   HeaderFooterSettings
	Footer   HeaderFooterSettings
	Template *template.Template
}

type latexTemplates struct {
	Invoice templatesCollection
	UPO     templatesCollection
}

func parseTemplates(vip *viper.Viper) (templates latexTemplates, err error) {
	invoiceTemplatesPath := vip.GetString(cfgKeyLatexInvoiceTemplates)
	upoTemplatesPath := vip.GetString(cfgKeyLatexUpoTemplates)

	var isValid = false

	if invoiceTemplatesPath != "" {
		templates.Invoice, err = readTemplatesFromDirectory(invoiceTemplatesPath)
		isValid = err == nil

		templates.Invoice.Header.Left = vip.GetString(cfgKeyLatexInvoiceHeaderL)
		templates.Invoice.Header.Center = vip.GetString(cfgKeyLatexInvoiceHeaderC)
		templates.Invoice.Header.Right = vip.GetString(cfgKeyLatexInvoiceHeaderR)

		templates.Invoice.Footer.Left = vip.GetString(cfgKeyLatexInvoiceFooterL)
		templates.Invoice.Footer.Center = vip.GetString(cfgKeyLatexInvoiceFooterC)
		templates.Invoice.Footer.Right = vip.GetString(cfgKeyLatexInvoiceFooterR)
	}

	if upoTemplatesPath != "" {
		templates.UPO, err = readTemplatesFromDirectory(upoTemplatesPath)
		isValid = err == nil
	}

	if err == nil && !isValid {
		err = errNoTemplatesDefined
	}

	return templates, err
}

var funcs = template.FuncMap{
	"add": func(value int) int {
		return value + 1
	},
	"float2string": func(value float64) string {
		return fmt.Sprintf("%.0f", value)
	},
	"replace_underline": func(value string) string {
		return strings.ReplaceAll(value, "_", "\\_")
	},
	"escape_percent": func(value string) string {
		return strings.ReplaceAll(value, "%", "\\%")
	},
	"grosz2pln": func(value int) string {
		return fmt.Sprintf("%.2f", float64(value)/100)
	},
	"map": func(pairs ...any) (map[string]any, error) {
		if len(pairs)%2 != 0 {
			return nil, errors.New("misaligned map")
		}

		m := make(map[string]any, len(pairs)/2)

		for i := 0; i < len(pairs); i += 2 {
			key, ok := pairs[i].(string)

			if !ok {
				return nil, fmt.Errorf("cannot use type %T as map key", pairs[i])
			}
			m[key] = pairs[i+1]
		}
		return m, nil
	},
	"has": func(a_map map[string]any, key string) bool {
		_, exists := a_map[key]
		return exists
	},
	"datetime_hr": func(timestamp time.Time) string {
		return timestamp.Format(`2006-01-02\\15:04:05 MST`)
	},
}

func readTemplatesFromDirectory(dirname string) (templatesCollection, error) {
	var templates = templatesCollection{
		Enabled: true,
		SrcPath: dirname,
	}

	var err error

	templates.Template, err = template.New("").Funcs(funcs).Delims("<<", ">>").ParseGlob(filepath.Join(dirname, "*.tex"))

	return templates, err
}
